...
/**
  *
  */
public class DynamicUpdateUtils implements DUConstants {
...
//            ex.printStackTrace();
    ...
    private static int offsetToEOL(String data, int pos) {
        ...
        for ( ; pos+i < count &&
            !Character.isISOControl(data.charAt(pos+i)); i++) {
            // nothing here
        }
        return i;
    }
...
//*/
    ...
    /** Compare local and remote hashtables and return a hashtable
      ...
      */
    public static Hashtable<String, DUOperation> compareHashtables(Hashtable local, Hashtable remote) {
        Hashtable<String, DUOperation> retval = new Hashtable<String, DUOperation>();
        ...
        for (Enumeration e=local.keys(); e.hasMoreElements(); ) {
            ...
            if (remoteObj == null) {
                ...
                if (op.remoteVersion.doubleValue() > op.localVersion.doubleValue()) {
                    op.operation = DUConstants.OP_UPDATE;
                }
            } // endif
...
//            op.localPath = op.redirectPath; // one day...
            ...
            op.operation = DUConstants.OP_DOWNLOAD;
            retval.put(key, op);
        } // endfor
    ...
    public static Vector[] sortOperationsByOp(Hashtable ops, Vector ids) {
        ...
        while (enumer.hasMoreElements()) {
            ...
            if (ids == null
             || ids.contains(k)) {
                DUOperation op = (DUOperation)ops.get(k);
    ...
    /**
      ...
      *   various operations.
      */
    public static int performOperations(DUConfig cfg, int type, Vector ops, DUProgressListener l) throws InterruptedException {
        int errorCnt = 0;
        int size = ops.size();
        for (int i=0; i<size; ++i) {
            DUOperation op = (DUOperation)ops.elementAt(i);
            if (type == TYPE_ALL
             || type == op.type) {
    ...
    /**
      ...
      */
    public static boolean performOperation(DUConfig cfg, DUOperation op, DUProgressListener l) throws InterruptedException {
        ...
        try {
            ...
            switch (op.operation) {
              case DUConstants.OP_UPDATE:
                ...
                String temp = new File(n).getParent()+File.separator+cfg.getAppShortName()+op.fileID+"_DynUpd.tmp";
...
//                File f = File.createTempFile("GTCS","DynUpd"); // 1.3 specific...
            ...
            if (op.operation != DUConstants.OP_NOTHING // no file chgs on nothings...
             && cfg != null) {
                // update config info:
...
//*/
    ...
    /** Counts the total download size (in bytes) of downloads and
      ...
      *   an error occurred.
      */
    public static int countDownloadSize(DUConfig cfg, int type, Vector ids) {
        int total = 0;
        try {
            Enumeration enumer = cfg.getOperations().elements();
            while (enumer.hasMoreElements()) {
                ...
                if (type == TYPE_ALL
                 || type == op.type) {
                    ...
                    if (ids == null
                     || ids.contains(op.fileID)) {
                        // in vector, or vector null
                        if (op.operation == DUConstants.OP_UPDATE
                         || op.operation == DUConstants.OP_DOWNLOAD) {
    ...
    /** Performs in the order: New downloads, Update files, Delete files.
      ...
      *  user intervention).
      */
    public static int performAllOperations(DUConfig cfg, int type, Vector ids, DUProgressListener l) throws InterruptedException {
        try {
            int errorCnt;
            Vector[] opSets  = sortOperationsByOp(cfg.getOperations(), ids);
            errorCnt  = performOperations(cfg, type, opSets[DUConstants.OP_DOWNLOAD], l);
            errorCnt += performOperations(cfg, type, opSets[DUConstants.OP_UPDATE  ], l);
            errorCnt += performOperations(cfg, type, opSets[DUConstants.OP_DELETE  ], l);
    ...
    /**
      ...
      */
    public static int getFileFromURL(URL rmt, File local, DUProgressListener l) throws InterruptedException {
        InputStream is  = null;
...
//System.out.println(ex);
    ...
    public static boolean startApplication(DUConfig cfg) {
        return startApplication(cfg.get(INI_EXEC_NAME),
                                cfg.get(INI_EXEC_ARGS),
                                cfg.get(INI_LOCAL_BASE));
    }
...
//System.out.println("duu.iPE: parent of "+filename+" is "+parent);
    ...
    public static void removeNonExistantFiles(Hashtable filesHash, String base) {
        ...
        while (enumer.hasMoreElements()) {
            ...
            DUOperation op = new DUOperation(k.toString());
